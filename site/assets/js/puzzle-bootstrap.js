...
async function loadLegacy(){
  ...
  document.querySelectorAll('[data-static-aria-i18n]').forEach(el=>{const value=staticTranslations[locale]?.[el.dataset.staticAriaI18n]??staticTranslations.ru[el.dataset.staticAriaI18n];if(value!==undefined)el.setAttribute('aria-label',value)});
  document.querySelectorAll('#puzzleMode option,#puzzleDifficulty option').forEach(option=>{option.textContent=window.RUDNI18N.t(option.textContent)});
...
async function localAdm1(iso,local){
  ...
  return wrapper({id:`${iso.toLowerCase()}-adm1-geoboundaries`,
    title:`${country.name||iso}: регионы`,title_en:`${country.name_en||country.name||iso}: regions`,title_zh:`${country.name_zh||country.name_en||iso}：行政区`,
    source,source_en:source,source_zh:source,license,license_en:license,license_zh:license,
    year:String(local.year||country.year||''),origin:'bundled',
    geometry_url:`${base}/adm1/${iso}.geojson`,feature_count:local.features});
}
   ...
   const work=(async()=>{
     if(snapshot.finished){
       const response=await puzzleFetch('/api/puzzle/complete',{method:'POST',body:JSON.stringify({
         ...
         placed:snapshot.placed,total:snapshot.featureIds?.length||snapshot.pieces?.length,hints:snapshot.hints,
