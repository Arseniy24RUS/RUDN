const CACHE='rudn-gmu-pages-v1.0.4';
...
self.addEventListener('fetch',event=>{
  ...
  const isData=/\.(?:json|geojson|topojson)$/i.test(url.pathname);
  if(isData){event.respondWith(fetch(event.request).then(response=>{const copy=response.clone();caches.open(CACHE).then(c=>c.put(event.request,copy));return response}).catch(()=>caches.match(event.request)));return}
