...
function decorateResolution(record,state){
 ...
 host.innerHTML=`<section class="result-short-story">${p?G.IllustratedAssets.tag(G.IllustratedAssets.image(p,{thumb:true}),{lazy:false}):'<img src="assets/illustrated/region.webp" alt="">'}<div><small>${esc(M.local(name,lang)||txt(lang,'Новая область','Novaya Oblast'))}</small><h3>${status}</h3><p>${timing}</p>${risk?`<p class="result-alert">${risk}</p>`:''}</div></section><div class="result-now-values">${[['coins',txt(lang,'Казна','Treasury'),record.effects.budget],['people',txt(lang,'Поддержка','Support'),record.effects.support],['development',txt(lang,'Развитие','Development'),record.effects.development]].map(([i,label,v])=>`<div>${G.icon(i,{size:19})}<span>${label}</span><b>${Number(v)>0?'+':''}${new Intl.NumberFormat(lang==='en'?'en-US':'ru-RU',{maximumFractionDigits:1}).format(Number(v)||0)}</b></div>`).join('')}</div><p class="result-year-note">${txt(lang,'После решения выполнен годовой пересчёт. Здесь также учтена работа прежних программ; кризис при входе в год показан отдельно.','The annual update includes earlier programmes as well as this decision. The crisis on entering the year is shown separately.')}</p>`;
 host.append(old);
}
