(function(root){
 ...
 const txt=(ru,en)=>lang()==='ru'?ru:en;
 const l=x=>E.localise(x,lang());
 const n=x=>new Intl.NumberFormat(lang()==='ru'?'ru-RU':'en-US',{maximumFractionDigits:2}).format(x);
 ...
 function briefing(s,language){
  ...
  let html='';
  if(due.length)html+=`<button type="button" class="gov-brief pending" data-open-promises>${icon('calendar',{size:17})}<span>${language==='ru'?`Есть обещание со сроком ${year(s,due[0].dueTurn)} года`:`A promise is due in ${year(s,due[0].dueTurn)}`}</span>${icon('arrow',{size:15})}</button>`;
  else if(scene)html+=`<div class="gov-brief">${icon('people',{size:17})}<span>${language==='ru'?'Этот проект обсудим с муниципалитетом и жителями.':'We will discuss this project with the municipality and residents.'}</span></div>`;
  if(E.getCurrentMission(s)?.id==='public-audit'){const q=G.summary(s);html+=`<button class="gov-brief" type="button" data-open-promises>${icon('journal',{size:17})}<span>${language==='ru'?`К публичному разбору: выполнено ${q.kept+q.revisedKept}, нарушено ${q.broken}, ещё в работе ${q.pending}.`:`Public review: ${q.kept+q.revisedKept} kept, ${q.broken} broken, ${q.pending} pending.`}</span>${icon('arrow',{size:15})}</button>`;}
  return html;
 }
