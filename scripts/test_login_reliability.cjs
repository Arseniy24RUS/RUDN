...
(async()=>{
  ...
  try{
    ...
    await page.evaluate(async()=>{await (await import('/assets/js/backend.js?v=1.3.2')).backend.init()});
    ...
    await page.evaluate(async()=>{
      const {backend}=await import('/assets/js/backend.js?v=1.3.2');const original=backend.lookupStudent.bind(backend);
