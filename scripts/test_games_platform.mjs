...
try{for(const engine of(process.env.DURABLE_TEST_BROWSERS||'chromium,webkit').split(',')){
 ...
 try{
  ...
  await page.locator('a[href="#games"]').last().click();await page.locator('[data-game]').first().waitFor();
  ...
  for(const key of ['lecture-1-test','topic-1']){
   ...
   await page.goto(base+'/#activity/lecture-1/test');await ready(page);assert.equal(await page.locator('#quizMount').count(),0);
   await page.goto(base+'/#dashboard');await ready(page);assert.equal(await page.locator('[data-lecture-test="lecture-1"]').isDisabled(),true);
  ...
  assert.deepEqual(errors,[]);await context.close();console.log(`PASS ${engine}: guest catalog/lazy modules/locales/history, actual5/5+5/5+50/50→three stars/gold, lower retake, direct gates`);
